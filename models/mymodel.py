...
class Model(nn.Module):
    def __init__(self, arch=None):
        ...
        for key in arch:
            ...
            if i=="0":
                if sum(arch[key])>0:
                    self.layerDict["layer_{}_{}".format(i, j)] = Layer(self.numOfInnerCell, 0, featureMap["f"+i]["channel"], featureMap["f"+j]["channel"], 4, cellArchPerLayer=arch[key], layerName=key, InnerCellArch=arch[key])
            else:
                if sum(arch[key])>0:
                    self.layerDict["layer_{}_{}".format(i, j)] = Layer(self.numOfInnerCell, 0, featureMap["f"+i]["channel"], featureMap["f"+j]["channel"], 1, cellArchPerLayer=arch[key], layerName=key, InnerCellArch=arch[key])
        # #info define network structure
        # self.layerDict = nn.ModuleDict({})
